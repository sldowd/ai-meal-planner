...
flowchart LR
    subgraph UI["🧑‍💻 User Interface Layer"]
        CLI[CLI - MVP]
        WEB[Web/Mobile UI - Future]
    end

    subgraph APP["🧠 Application Layer"]
        UserSvc[User Profile Service]
        PlanSvc[Meal Planning Service]
        RecipeSvc[Recipe Management Service]
        Orchestrator[Application Orchestration Service]
    end

    subgraph DOMAIN["📦 Domain Layer"]
        ProfileModel[Profile Domain Model]
        PlanModel[Meal Plan Model]
        RecipeModel[Recipe Domain Model]
    end

    subgraph AI["🤖 AI/ML Layer"]
        Recommender[Recommendation Engine]
        RecipeGen[Recipe Text Processor]
    end

    subgraph DATA["🗃️ Data Access Layer"]
        ProfileRepo[Profile Repository]
        PlanRepo[Meal Plan Repository]
        RecipeRepo[Recipe Repository]
        SQLAlchemy[SQLAlchemy ORM]
        Dataset[Recipe Dataset optional]
    end
    ...
    PlanSvc --> Orchestrator
    RecipeSvc --> Orchestrator

    %% APP -> DOMAIN
    Orchestrator --> ProfileModel
    Orchestrator --> PlanModel
    Orchestrator --> RecipeModel

    %% AI connection
    Orchestrator --> Recommender
    Recommender --> RecipeGen

    %% Domain <-> Repos
    ProfileModel --> ProfileRepo
    PlanModel --> PlanRepo
    RecipeModel --> RecipeRepo

    %% Repos -> DB
    ProfileRepo --> SQLAlchemy
    PlanRepo --> SQLAlchemy
    RecipeRepo --> SQLAlchemy
    SQLAlchemy --> DB
    Dataset --> RecipeGen
